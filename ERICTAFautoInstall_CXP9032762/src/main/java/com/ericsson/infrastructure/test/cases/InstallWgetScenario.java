...
public class InstallWgetScenario extends TorTestCaseHelper {

   
	@Inject
	InstallWgetTestSteps steps;
	
    @TestId(id = "install_wget_on_server", title = "Install custom compiled wget on server")
...
public void createTestFlow() {
      ...
      testFlow = flow("installWget")
            
             .addTestStep(annotatedMethod(steps, "copyUntarWget"))
             .build();
      
      TestScenario scenario = scenario("copyUntarWget").addFlow(testFlow).build();
