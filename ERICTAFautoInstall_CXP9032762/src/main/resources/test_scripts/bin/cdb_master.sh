...
# Using below instead of above as sym linking to CLOUD_xlaxain so that
# can cope with using sym link to that directory
...
MOUNTPOINT="$( cd -P "$( dirname "$SOURCE" )" && pwd )"

# Now resume as normal
...
message "INFO: At the end of adm1_part1" INFO
message "INFO: At the end of adm1_part1 ... Sleeping.." INFO
sleep 600
ssh ossmaster "pkginfo -l ERICscs;/opt/ericsson/nms_cif_sm/bin/smtool -list scs pss"
	if [[ $? -eq 0 ]]
	then
...
function set_eeprom_text_prefix ()
{
	...
	$SSH -qt $X_HOSTNAME "$MOUNTPOINT/bin/set_eeprom_text.sh" 2>/dev/null
        $SSH -qt $X_HOSTNAME "/usr/sbin/bootadm generate-menu -f"
}
       ...
       # echo "INFO: Pinging $SERVER"
        ...
        if [ $? -eq 0 ]; then
            # echo "INFO: $SERVER is alive"
       	    setup_passwordless_ssh $SERVER $SSHUSER $SSHPASS
            if [ $? -ne 0 ]; then
	        message "ERROR: Failed to setup passwordless ssh to $SERVER\n" ERROR
		$ACTION
            #else
...
EOF
                ...
                if [ $? -eq 0 ] ;then
                        message "INFO: scripts already mounted\n" INFO
                        return 0
		fi
...
ssh ossmaster "pkginfo -l ERICscs;/opt/ericsson/nms_cif_sm/bin/smtool -list scs pss"  
	...
	if [[ "$OMSAS_HOSTNAME" != "" ]] && [[ "$INITIAL_INSTALL_OMSAS" == "yes" ]] && [[ "$INITIAL_INSTALL_OMSAS_PART2" != "no" ]]
		...
		LOG_FILE=$RUNNING_LOG_DIR/$PARALLEL_ID.log
			...
			#swap space workaround moved up to before csa_omsas
                        #$SSH -qt $OMSAS_HOSTNAME "zfs set volsize=10g rpool/swap; zfs get volsize rpool/swap " > /dev/null 2>&1
